...
This is the engineering plan: what to build, in what order, with the concrete technical
decisions each part needs. It assumes the product spec (segments, goals, non-goals) as
...
Explicitly **out** of v1: WhatsApp, live-class copilot, pre-purchase sales mode, native
mobile app, helpdesk migration tooling.
...
| LLM | **Claude** — `claude-sonnet-5` for answers, `claude-haiku-4-5` for classification/tagging/summaries, `claude-opus-5` where quality dominates | Cost math and the "validate the tiering with evals" caveat in §14 |
...
This is the part that decides whether the bot is any good.
...
Step 7 is where you catch bad answers before customers do, which is why go-live is gated on
it. Keep the gate.
...
|---|---|---|
| Classification + tagging (6× · ~600 in / ~50 out each) | `claude-haiku-4-5` ($1/$5 per MTok) | ~$0.005 |
| Answers (6× · ~5K in / ~300 out each) | `claude-sonnet-5` ($3/$15 per MTok) | ~$0.12 |
| Same, with prompt caching on the stable prefix (~3K of the 5K cached at 0.1×) | | **~$0.07** |
| Summary + suggestions (agent-escalated only) | `claude-haiku-4-5` | ~$0.002 |
| Embeddings (ingest, amortized) | Voyage | negligible |

**Roughly $0.07 per AI-resolved conversation** with caching in place — a brand doing 3,000
conversations/month costs you about $210 in inference. That's the number to price against;
it comfortably supports the ₹4,999-for-1,000-conversations floor in §16.

Two pricing caveats worth pinning down before you build the model on it:

- Sonnet 5 currently has **introductory pricing of $2/$10 per MTok, which ends 2026-08-31.**
  Every figure above uses the standard $3/$15 rate, so the plan doesn't quietly depend on a
  discount that expires in under two weeks. If you model near-term margin on the intro rate,
  label it as temporary.
- `claude-opus-5` is $5/$25 per MTok — roughly 1.7× Sonnet 5 on both sides. Use it
  selectively (complex escalation decisions, KB gap analysis), not on the main answer path.

The Sonnet-for-answers / Haiku-for-classification split is a deliberate cost tradeoff, not a
default. Validate it with the eval harness (§5) before locking it in: if Sonnet 5 costs you
several points of resolution rate against Opus 5, the extra inference spend is cheaper than
the escalations, and you should move the answer path up a tier.
...
These are the open questions, with my recommendations so you can start:
