...
import { pgTable, text, jsonb, timestamp, uuid, boolean, smallint, index, uniqueIndex, pgEnum } from 'drizzle-orm/pg-core';
...
export const messages = pgTable('messages', {
  ...
  // RFC 5322 Message-ID of this message, stored WITHOUT angle brackets.
  // Set for inbound mail (from the provider payload) and for outbound mail
  // (generated by us before sending) so reply chains resolve in both directions.
  emailMessageId: text('email_message_id'),
  emailInReplyTo: text('email_in_reply_to'),
  // Space-separated References header, stored without angle brackets.
  ...
  // Idempotency guard for webhook redelivery: a provider retrying the same
  // inbound email cannot create a second message row in the same workspace.
  uniqueIndex('messages_workspace_email_message_id_key')
    .on(t.workspaceId, t.emailMessageId)
    .where(sql`${t.emailMessageId} is not null`),
]);
